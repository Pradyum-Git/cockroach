...
var dbworkloadMeta = workload.Meta{
	...
	New: func() workload.Generator {
		...
		g.flags.BoolVar(&g.anonymise, "anonymise", false,
			"When set, anonymize all table and column names in both DDL and SQL.")
...
type workloadGeneratorStruct struct {
	...
	mapping        Mapping
	anonymise      bool // whether to apply anonymization
...
func (w *workloadGeneratorStruct) Hooks() workload.Hooks {
	return workload.Hooks{
		...
		PreCreate: func(db *gosql.DB) error {
			...
			schemas, stmts, mapping, err := generateDDLs(w.debugLogsLocation, origDbName, w.anonymise)
			if err != nil {
				return errors.Wrap(err, "failed to generate DDLs from debug logs")
			}
			w.allSchema, w.createStmts, w.mapping = schemas, stmts, mapping
			...
			if err := generateWorkload(w.debugLogsLocation, w.allSchema, w.dbName, origDbName, mapping, w.anonymise, w.outputDir); err != nil {
