...
func writeTransaction(
	...
	for _, txnID := range txnOrder {
		...
		_, writeError = fmt.Fprintln(outFile, "-------Begin Transaction------")
		_, writeError = fmt.Fprintln(outFile, "BEGIN;")
		for _, stmt := range stmts {
			_, writeError = fmt.Fprintf(outFile, "%s;\n", stmt)
		}
		_, writeError = fmt.Fprintln(outFile, "COMMIT;")
		_, writeError = fmt.Fprintln(outFile, "-------End Transaction-------")

		if writeError != nil {
			return errors.Wrapf(writeError, "writing transaction %s to %s", txnID, outFile.Name())
		}
	}
	return nil
}
