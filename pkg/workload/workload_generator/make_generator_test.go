...
func TestSequenceGenerator(t *testing.T) {
	...
	gen := makeSequenceGenerator(col, 2, 10)
...
func TestIntegerAndFloatGenerators(t *testing.T) {
	...
	igen := makeIntegerGenerator(icol, rng)
	for i := 0; i < 10; i++ {
		v := igen.Next()
	...
	fgen := makeFloatGenerator(fcol, rng)
	for i := 0; i < 10; i++ {
		v := fgen.Next()
...
func TestTimestampGenerator(t *testing.T) {
	...
	rng := rand.New(rand.NewSource(0))
	gen := makeTimestampGenerator(col, rng)
	v := gen.Next()
...
func TestStringBoolJsonGenerators(t *testing.T) {
	...
	sgen := makeStringGenerator(scol, rng)
	for i := 0; i < 5; i++ {
		v := sgen.Next()
...
func TestMakeGeneratorAllTypes(t *testing.T) {
	...
	for _, tt := range types {
		gen := makeGenerator(ColumnMeta{Type: tt.typ, Args: tt.args}, 0, 1, schema)
		v := gen.Next()
