...
const (
	...
	// alphaLetters is the alphabet used by StringGen.
...
type UniqueWrapper struct {
	base     Generator           // the underlying generator to delegate to
	seen     map[string]struct{} // a set of recently seen values
	order    []string            // a queue of the order in which values were seen
	capacity int                 // the max number of recent values to remember
}
...
type FkWrapper struct {
	parent  Generator // the base generator to repeat values from
